...
public class WindsweptLootModifierProvider extends LootModifierProvider {
    ...
    public WindsweptLootModifierProvider(GatherDataEvent event) {
        super(event.getGenerator().getPackOutput(), event.getLookupProvider(), Windswept.MOD_ID);
    }

    @Override
    protected void registerEntries(HolderLookup.Provider provider) {
        this.add("goat_meat", "entities/goat", new LootPoolsModifier(Collections.singletonList(LootPool.lootPool().name("windswept:goat").setRolls(ConstantValue.exactly(1f))
                .add(LootItem.lootTableItem(WindsweptItems.GOAT.get())
                        ...
                        .apply(SmeltItemFunction.smelted().when(LootItemEntityPropertyCondition.hasProperties(LootContext.EntityTarget.THIS,
                                EntityPredicate.Builder.entity().flags(EntityFlagsPredicate.Builder.flags().setOnFire(true).build()).build())))).build()), false));

        this.add("drowned_disc", "entities/drowned", new LootPoolsModifier(
                Collections.singletonList(LootPool.lootPool().name("windswept:rain_disc")
                        ...
                        .when(LootItemEntityPropertyCondition.hasProperties(LootContext.EntityTarget.ATTACKER,
                                EntityPredicate.Builder.entity().of(EntityTypeTags.SKELETONS).build())).build()), false));

        this.add("stray", "entities/stray", new LootPoolsModifier(List.of(
                LootPool.lootPool().name("minecraft:arrow").setRolls(ConstantValue.exactly(1f)).add(LootItem.lootTableItem(Items.ARROW)
                        ...
                        .apply(EnchantedCountIncreaseFunction.lootingMultiplier(provider, UniformGenerator.between(0f, 1f)))).build(),
        ...
        this.add("village_savanna_house", "chests/village/village_savanna_house", new LootPoolsModifier(List.of(LootPool.lootPool().name(Windswept.MOD_ID + ":village_savanna_house").setRolls(UniformGenerator.between(1f, 3f))
    ...
    private void chestEntry(String name, String target, DeferredHolder<net.minecraft.world.item.Item, ? extends ItemLike> item, Consumer<LootPoolSingletonContainer.Builder<?>> b) {
        LootPoolSingletonContainer.Builder<?> builder = LootItem.lootTableItem(item.get());
        b.accept(builder);
        this.add(name, target, new LootPoolsModifier(Collections.singletonList(
                LootPool.lootPool().name(Windswept.MOD_ID + ":" + name).setRolls(ConstantValue.exactly(1f)).add(builder).build()), false));
    }
